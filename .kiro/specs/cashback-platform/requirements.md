...
2. THE SYSTEM SHALL only mark a UID link `verified` if that UID appears in an imported,
   ...
   For the Bybit MVP, an admin must also confirm the claimant controls that UID and
   record the ownership evidence reference; report membership alone is insufficient.
...
6. WHEN a previously unknown UID later appears in an imported report, THE SYSTEM SHALL
   re-evaluate matching pending link requests and verify those with approved ownership.
