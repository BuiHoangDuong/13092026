...
export function WithdrawalFlow({ exchanges, routes, initial, exchangeId: seedExchange, uid: seedUid }: { exchanges: Array<{ id: string; name: string }>; routes: PayoutRoutes; initial: Principal | null; exchangeId: string; uid: string }) {
  ...
  return <div className="space-y-8"><h1 className="text-3xl font-bold">{m.title}</h1><p className="text-muted-foreground">{m.firstReview}</p>
    ...
    </form>:<form onSubmit={verify} className="mt-5 space-y-4"><label className="block text-sm">{m.code}<InputOTP required disabled={busy} maxLength={6} value={code} onChange={setCode} autoComplete="one-time-code" containerClassName="mt-2"><InputOTPGroup>{Array.from({length:6},(_,index)=><InputOTPSlot key={index} index={index}/>)}</InputOTPGroup></InputOTP></label><p className="text-xs text-muted-foreground">{m.expires}: {otp.expiresAt}</p><Button disabled={busy || code.length !== 6}>{m.verify}</Button><Button disabled={busy} type="button" variant="secondary" onClick={()=>{setOtp(null);setCode("");}}>{m.resend}</Button></form>}</section>:<>
