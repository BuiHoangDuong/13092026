...
export const POST = (request: Request) => withAdmin(async () => adminJson({ exchange: await createExchange(await parseAdminRequest(request, adminExchangeCreateSchema)) }, { status: 201 }));
export const PATCH = (request: Request) => withAdmin(async () => adminJson({ exchange: await updateExchange(await parseAdminRequest(request, adminExchangeUpdateSchema)) }));
