...
export const POST = (request: Request) => withAdmin(async () => adminJson({ guide: await createGuide(await parseAdminRequest(request, adminGuideCreateSchema)) }, { status: 201 }));
export const PATCH = (request: Request) => withAdmin(async () => adminJson({ guide: await updateGuide(await parseAdminRequest(request, adminGuideUpdateSchema)) }));
